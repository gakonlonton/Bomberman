...
public class Map {
    ...
    protected List<List<Entity>> grass = new ArrayList<>();
    private Graph graph;
    ...
    public void mapReader() {
        ...
        for (int i = 0; i < HEIGHT; i++) {
            ...
            grass.add(new ArrayList<>());
            for (int j = 0; j < WIDTH; j++) {
                grass.get(i).add(new Grass(j, i, Sprite.grass.getFxImage()));
    ...
    public void mapRender(GraphicsContext gc) {
        for (int i = 0; i < grass.size(); i++) {
            for (int j = 0; j < grass.get(i).size(); j++) {
                grass.get(i).get(j).render(gc);
            }
        }
        for (int i = 0; i < map.size(); i++) {
            for (int j = 0; j < map.get(i).size(); j++) {
                if (!(map.get(i).get(j) instanceof Grass)) {
